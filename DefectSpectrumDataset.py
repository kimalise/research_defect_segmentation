...
def build_meta(root_dir):
    ...
    mask_ext = os.path.splitext(mask_filename_list[0])[1]
    rbg_mask_ext = os.path.splitext(rbg_filename_list[0])[1]
    ...
    for image_file in image_filename_list:
        ...
        mask_file_name = main_file_name + "_mask" + mask_ext
        rbg_mask_file_name = main_file_name + "_rgb_mask" + rbg_mask_ext

        if mask_file_name not in mask_filename_list:
            print("cannot find {}".format(mask_file_name))
            continue

        if rbg_mask_file_name not in rbg_filename_list:
            print("cannot find {}".format(rbg_mask_ext))
            continue

        result.append({
            "image": os.path.join(image_path, image_file),
            "mask": os.path.join(mask_path, mask_file_name),
            "rbg_mask": os.path.join(rbg_mask_path, rbg_mask_file_name)
        })
...
def split_data(meta_json, train_rate):
    ...
    return train_meta_json.tolist(), val_meta_json.tolist()

def save_json(data, file_path):
    # 打开文件并将 data 保存为 JSON 格式
...
class DefectSpectrumDatasetV2(Dataset):
    def __init__(self, meta_json_path, transform=None):
        ...
        self.transform = transform
        self.example = [(example['image'], example['mask'], example['rbg_mask']) for example in self.meta_json]
    ...
    def __getitem__(self, index):
        ...
        rbg_mask = cv2.imread(self.example[index][2], cv2.IMREAD_UNCHANGED)
        ...
        return aug_image, aug_mask, rbg_mask
